    ...
    B: Serialize + std::fmt::Debug,
{
    ...
    if let Some(token) = get_access() {
        req = req.header("Authorization", &("Bearer".to_owned() + &token));
    }

    if allow_body {
        let body_json = serde_json::to_string(&body)
            .map_err(|ex| Error::SerdeJson(format!("Serialize error: {ex}")))?;
    ...
    let response = req
        .send()
        .await
        .map_err(|ex| Error::Network(format!("Http request error{ex}")))?;
    ...
    T: DeserializeOwned + 'static + std::fmt::Debug,
{
    if response.ok() {
        ...
        match status {
            ...
            _ => Err(Error::Network(format!("Unexpected status code: {status}",))),
        }
    }
}
